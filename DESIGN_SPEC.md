# PACKLOG 行前志 — Design Specification
> 给 Cursor 的前端设计规范文档  
...
```
[序号 idx]  [状态圆点]                    [操作按钮组 hover显示]
[城市粗体] · [地点/场景 斜体]
[日期]  ·  [场景标签]  ·  [件数]
[进度条 1.5px]                              [百分比]
```

**序号**：EB Garamond italic，10px，--ink4，格式 "01" "02"
**状态圆点**：5px 实心圆，打包中=--forest3，已归档=--ink4，不用文字标签
**行程标题**：EB Garamond，21px，城市粗体 + 地点斜体，hover 变 --forest2
**分隔符**：用 · 中点，不用 - 连字符
**日期/标签/件数**：10px，DM Sans，--ink4/--ink3，用 2px 圆点间隔
**进度条**：height 1.5px，连续线，不是分段方块，--paper3底+--forest3填充
**进度百分比**：EB Garamond italic，11px，--ink4，右对齐
**操作按钮**：默认 opacity:0，hover opacity:1；三个图标圆钮（归档/置顶/删除），28px，圆形，hover背景 --paper2

### 绝对禁止
- ❌ 进度条做成分段方块
- ❌ "打包中"状态文字出现两次（左上角+日期旁）
- ❌ 归档/置顶/删除三个文字按钮常驻显示
- ❌ 行程标题用连字符 - 分隔
...
.item.done .item-name { text-decoration: line-through; text-decoration-color: var(--ink4); }
```

**勾选圆**：17px，border: 1px solid --paper3；勾选后 background: --forest2
**物品名**：13.5px，DM Sans 400，--ink
**品牌备注**：EB Garamond italic，12px，--ink3
**说明文字**：11px，DM Sans，--ink3，line-height 1.5
**状态标签**：9px，DM Sans 500，UPPERCASE，border-radius 4px，padding 2px 7px

状态标签色值：
- 必带：background #E8F2E4，color #3A5C33
- 待购买：background #F5ECD8，color #9B6A2A
- 可选：background #E8E3D8，color #8C8880
- 容器标签：background #E8E3D8，color #4A4840
...
深色背景（Hero Card等）：currentColor = #FEFCF8
浅色背景：currentColor = #243D1F

---

## 十三、全局筛选与下拉控件规范（v3.1 新增）
...
- 筛选 chip（默认/激活）
- 输入框（搜索/关键词）
- 下拉触发器（select trigger）
- 下拉面板（列表容器）
- 选项项（默认/hover/selected）

禁止同项目内出现“同类控件字号、圆角、边框、选中色”不一致的情况。

### 13.2 统一尺寸与排版

- 筛选 chip：高度 `32px`，左右内边距 `12px`，字号 `11px`
- 输入框：高度 `40px`，字号 `14px`
- 下拉触发器：高度 `40px`，字号 `14px`
- 下拉选项：高度建议 `32-36px`，字号 `14px`
- 分类小标题：`11px`，`letter-spacing: .06em`，色值 `--ink3`

### 13.3 统一颜色与状态

- 默认 chip：`background: var(--white)`，`border: .5px solid var(--paper3)`，`color: var(--ink2)`
- 激活 chip：`background: var(--forest)`，`border-color: var(--forest)`，`color: #FEFCF8`
- 输入框/触发器：`background: var(--white)`，`border: .5px solid var(--paper3)`，文字 `--ink2`
- 下拉 hover 行：`background: var(--paper2)`
- 下拉 selected 行：`background: var(--forest5)`，文字 `--forest`

### 13.4 交互规范

- 点击输入框/触发器本体即可展开列表，不依赖只点右侧箭头
- 展开后第一行提供搜索输入（若列表长度 > 8）
- 搜索结果按业务分组展示（如品牌按“户外服饰/鞋靴/装备/美妆个护/电子”）
- 点击选项后立即关闭面板并写入值
- 允许手动输入的字段需保留自由输入能力

### 13.5 对齐规范

- 同一行的 chip、按钮、输入框基线对齐
- 文本垂直居中，不出现“按钮文字比输入框文字大一号”现象
- 页面内同语义操作文案统一（如“筛选”“清空”“添加”）

### 13.6 筛选状态提示规范（全局）

- 所有带筛选动作的列表区，筛选条下方必须有一行小字状态提示
- 提示字号统一 `11px`，颜色默认 `--ink3`
- 三种状态必须覆盖：
  - 默认：展示“显示全部 N 条/件”
  - 有筛选且有结果：展示“筛选结果：X / Y”
  - 有筛选且无结果：展示“未找到匹配结果，请调整筛选条件”
- 清空筛选后，提示必须立即恢复默认态；不得等待页面刷新

### 13.7 视图切换与行操作按钮（移动端）

- 视图切换按钮禁止使用“视图”这种语义不完整文案，必须明确动作：
  - `切换为紧凑` / `切换为详细`
- 行项目右侧操作按钮（拖动、编辑）统一使用图标小按钮：
  - 尺寸 `30x30`
  - 图标尺寸 `16`
  - 圆角 `8px`
  - 仅图标，不使用大白框大文字按钮；默认透明底，hover 再显浅底
- 移动端行项目默认采用紧凑卡片感（非超长横条），保持信息可读且减少视觉占用
